...
try {
    ...
    $genre_code = $stmt_select->fetchAll(PDO::FETCH_COLUMN);
} catch (PDOException $e) {
    exit($e->getMessage());
}
?>
            ...
            <form action="create.php" method="post" class="registration-form">
                <div>
                    ...
                    <select id="genre_code" name="genre_code" required>
                        ...
                        foreach ($genre_code as $genre_code) {
                            echo "<option value='{$genre_code}'>{$genre_code}</option>";
                        }
                        ?>
                    </select>
                </div>
                <button type="subimit" class="submit-btn" name="submit" value="create">登録</button>
            </form>
        </article>
    </main>
    <footer>
